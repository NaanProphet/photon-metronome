...
//=================================================================
//The draw function.
//This is runs continuously until the application is stopped.
...
void draw() 
{
  ...
  textFont (f, 16);                
  fill(200);                        
  text ("MIDI to Spark Core", 75, 150);  
}
...
void rawMidi(byte[] data) 
{
  ...
  if (data[0] == MIDI_CLOCK_START || data[0] == MIDI_CLOCK_CONTINUE)
  {
  ...
  else if (data[0] == MIDI_CLOCK_TIMING) 
  {
    ...
    if (flashingLed == true)
    {
      ...
      //Create a multiplier value between 0 and 1 which
      //gets smaller as the midiTimingCounter gets bigger
      float multiplier = (24.0 - midiTimingCounter) / 24.0;
      ...
      boolean sent = udp.send(data_to_send, sparkCoreIpAddress, udpPort);
    }
  ...
  else if ((data[0] & MIDI_STATUS_NIBBLE) == MIDI_CC_STATUS)
  {
